...
activity_df %>% 
  ...
  labs(
    caption = "Fig.4 24-hour activity \"profiles\" for each day"
  ) +
  theme(
...
day of week on 24-hour activity profile (day of the week effect)
...
activity_df %>% 
  ...
  labs(
    y = "sum of activity counts",
    caption = "Fig.5 relationship between day of the week and 24-hour activity profile"
  ) +
  theme(
    axis.text.y = element_text(size = 7),
    plot.caption = element_text(hjust = 0, size = 10)
...
Here we can see `Friday` and `Sunday` are different from the other five days, the total activity counts are higher and have greater fluctuation.