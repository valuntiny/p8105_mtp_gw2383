...
Figure.2 showed the linear regression line and corresponding 95% confidence interval, the slope is positive: `r summary(lm(sum_byday ~ c(1:length(sum_byday)), cross_min))$coefficient[2,1]` (p.value = `r summary(lm(sum_byday ~ c(1:length(sum_byday)), cross_min))$coefficient[2,4]`), we then concluded that there is relationship between day and the total activity, this participant became more and more active over time.

Additionally, we could also try to figure our if day of the week affects total activity:
...
In Fig.3A, we can see some fluctuation over each weekday, like the total activity reached bottom on `Tuesday` and reached its peak on `Friday`. But actually, those weekdays' total count are in the same order of magnitude, so the differences may not be that prominent. Hence, we used `ANOVA`  to test the hypothesis, below is the ANOVA table:
...
So after anova, p.value >= 0.05, we conclude that day of the week (in isolation to the effect of time) doesn't affect activity. In Fig.3B, we add the effect of time, and it shows that within each week, there is also some fluctuation between weekdays. But still, counts are in the same order of magnitude, so the effect is not that prominent either.

## 24-hour profile

We use heatmap for 24-hour activity profiles:
...
Fig.5A showed that `Friday night` and `Sunday afternoon` are different from the other five days, the total activity counts are higher and have greater fluctuation. When following the time through 47 weeks, Fig.5B showed to us that each week's profile varies and usually `Sunday` and `Friday` are the most prominent among other weekdays
